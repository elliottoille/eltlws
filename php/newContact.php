...
function newContact() { # define the new function
    ...
    # SQL code that fetches the associated userID of the username entered by the user
    ...
    if ($userID1 > $userID2) { # If the userID1 is bigger than userID2
        ...
        # make each userID 8 digits long by padding the front with 0s and create a string with the smaller UID first
        ...
        # format the userIDs again and make the string with smaller UID first
        ...
        # If the userIDs are the same then the user is trying to add themselves as a contact which isn't allowed
        return; # end function if they do this
    }
    $sql = "INSERT INTO `userscontacts` ( `low`, `high`) VALUES ('$low', '$high');";
    # Insert the values of low and high into the userContacts table
    ...
    if (mysqli_num_rows($result) != 0) { # If the num of rows is not 0 then
        if ($row["Tables_in_eltlws"] == $tableName) { # if one of the results matches the table we want to create
            ...
            $sql = "CREATE TABLE `$tableName`(`messageID` INT NOT NULL AUTO_INCREMENT,
            `message` TEXT(65535) NOT NULL,
            `userID` INT NOT NULL,
            PRIMARY KEY (`messageID`),
            FOREIGN KEY (`userID`) REFERENCES `users`(`userID`));";
            $result = mysqli_query($conn, $sql);
            # otherwise create a new table with the name of the table we just generated
        } 
    } else {
        $sql = "CREATE TABLE `$tableName`(`messageID` INT NOT NULL AUTO_INCREMENT,
        `message` TEXT(65535) NOT NULL,
